...
def _get_float_input(message="") -> float:
    while True:
        raw_input = input(message)
...
def _get_matrix_element(row, col) -> float:
    return _get_float_input(f"Digite o elemento A[{row}][{col}] da matriz de custo: ")


def get_list(num_elements: int, element_label: str, message: str) -> list[float]:
    print(message)
    ...
    for i in range(num_elements):
        list_.append(
            _get_float_input(f"Digite a quantidade do {i + 1}º {element_label}: ")
        )

    return list_


def get_cost_matrix(num_rows: int, num_cols: int) -> list[list[float]]:
